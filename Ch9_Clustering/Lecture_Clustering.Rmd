...
接著探討個群體間的連續變數分配，為了將資料轉換成整齊的格式，這邊會用到`gather()`函數將所有的variables併入同一個欄位裡面
...
> 這邊只是用一個很簡單的方式，就是把Kmeans的分群結果，貼回去有原始值的GameTable而已，這樣就可以進行原始值的畫圖，「阿他們究竟買多少？」
...
1. 第一群的玩家較多是使用ABC，常常在下午玩，但是沒有顯著的購買行為
2. 第二群的玩家較多是使用非ABC，常常在晚上玩，且常常買金幣
3. 第三群的玩家較多是使用非ABC，有一定比例的老手，常常在晚上玩，且常常買卡片
4. 第四群的玩家較多是使用非ABC，常常在半夜玩，且常常買鑽石
