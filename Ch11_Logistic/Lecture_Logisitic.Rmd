...
- 註冊方式是用Mobile_app
- class_1的那群人
其他服務品質相關因素是否會影響其忠誠度的顯著程度。那若這群人變成Class_2或Class_3，並不會影響其顯著程度。
...
因此為什麼 `class2` 和 `class3` 在模型 3 中顯著有正面影響，模型 2 中卻沒有呢？可能就不是很難理解了。因為本質上他們就是兩群不同的，模型3的那群立足點族群，應該是比較不是「飛行常客」等等，而且信用卡的紅利也是最少，因此若是你有給他升等（好比說長榮無限萬哩遊，從綠卡變成金卡）那自然就有很大的影響。
...
## try the demo of function if it works
