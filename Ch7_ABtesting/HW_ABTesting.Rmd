...
```{r 實驗組與對照組的樣本數檢查}
analysis.table %>% filter(test == 1) %>% nrow()
analysis.table %>% filter(test == 0) %>% nrow()
```
...
t.test(x = analysis.table.JP[analysis.table.JP$test==1, ]$purchase_amount, ##我們是要比他們的購買金額有沒有顯著差異 
